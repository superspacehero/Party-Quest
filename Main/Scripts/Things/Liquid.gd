...
# How high is the liquid
@export_range(0, 1) var liquid_height: float = 1.0:
	set(value):
		liquid_height = value
		set_liquid_height(value)
# How high is the wobble
@export var max_wobble := 0.3
...
func set_liquid_height(value) -> void:
	material_override.set_shader_parameter("liquid_height", value)

# Set the wobble intensity
func set_wobble() -> void:
...
func _ready() -> void:
	...
	set_liquid_height(liquid_height)
	set_wobble()
...
func _process(delta: float) -> void:
	...
	var current_rotation_len := (global_rotation - _prev_rot).length()
	_prev_pos = global_transform.origin
	_prev_rot = global_rotation
# END: movement
	# ANCHOR: decay
